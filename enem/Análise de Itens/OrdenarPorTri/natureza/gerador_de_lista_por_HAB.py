...
def questionBalance_Hab(hab, dfResult):
    ...
    dfResult_CN['QSEARCH'] = dfResult_CN.apply(lambda row: get_prova_string(row['ANO'], row['CO_PROVA']), axis=1)
...
#CN
    ...
    baralho = genanki.Deck(
        ...
        str('Habilidades Natureza:: '+name) # O nome do baralho
    )
    ...
    for i in dfResult_CN.index:
        ...
        if 'dtype:' in strCN:
            ...
            try:
                ...
                pdf.ln(10)
                stringCorr = str("Questao " + str(dfResult_CN.loc[i, "CO_POSICAO"])+' Natureza ENEM '+str(dfResult_CN.loc[i, "ANO"]) +' '+ str(dfResult_CN.loc[i, "QSEARCH"]))
