...
def questionBalance_65(name, dfResult):
...
#CN
    ...
    for i in dfResult_CN.index:
        strLC ="Nº"+str(dfResult_CN.loc[i, 'indexacao'])+" - Q" + str(dfResult_CN.loc[i, "CO_POSICAO"])+':'+str(dfResult_CN.loc[i, "ANO"]) + ' - H'+str(dfResult_CN.loc[i, "CO_HABILIDADE"].astype(int))+ " - Proficiência: " + str(dfResult_CN.loc[i, "theta_065"].round(2)) + ' '+str(dfResult_CN.loc[i, "CO_PROVA"])
        if 'dtype:' in strLC:
            print("CaCNulando...")
        else:
            try:
    ...
    strOut = 'Saidas/' + 'natureza2022' + 'prova.pdf'            
...
dItens = dItens[dItens['ANO'] == 2022]

questionBalance_65(2022, dItens)