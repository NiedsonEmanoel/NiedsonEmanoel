...
#CH
    ...
    for i in dfResult_CH.index:
        print("Nº"+str(dfResult_CH.loc[i, 'indexacao'])+"/"+str(len(dfResult_CH)))
        strLC ="Nº"+str(dfResult_CH.loc[i, 'indexacao'])+" - Q" + str(dfResult_CH.loc[i, "CO_POSICAO"])+':'+str(dfResult_CH.loc[i, "ANO"]) + ' - H'+str(dfResult_CH.loc[i, "CO_HABILIDADE"].astype(int))+ " - Proficiência: " + str(dfResult_CH.loc[i, "theta_065"].round(2))
        if 'dtype:' in strLC:
            print("CaCHulando...")
        else:
            try:
