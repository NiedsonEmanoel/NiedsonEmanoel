...
#CH
    ...
    baralho = genanki.Deck(
        ...
        str('Habilidades Humanas:: '+name) # O nome do baralho
    )
    ...
    # Percorrer as linhas do dataframe dfResult_CH
    for i in dfResult_CH.index:
        # Obter o nome do arquivo de imagem da questão
        imagem = str(dfResult_CH.loc[i, "CO_ITEM"]) + '.png'
        caminho_imagem = os.path.join(pasta, imagem)
        
        # Obter a resposta da questão
        resposta = str(dfResult_CH.loc[i, 'TX_GABARITO']) 
        inic = "Q" + str(dfResult_CH.loc[i, "CO_POSICAO"]) + ':' + str(dfResult_CH.loc[i, "ANO"]) + ' - H' + str(dfResult_CH.loc[i, "CO_HABILIDADE"].astype(int)) + " - Proficiência: " + str(dfResult_CH.loc[i, "theta_065"].round(2))
